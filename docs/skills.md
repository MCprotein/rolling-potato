...
Phase 3 currently implements pre-execution normalization.
...
- `rpotato run "<request>"` maps user requests to skill/mode through deterministic intent rules and records a ledger event.
...
- Real model/backend execution, context packing, tool calls, and patch application happen in later phases.
