...
- Before generation, pinned `llama.cpp b9982` receives the same text, JSON schema, chat-template options, and image content at `/v1/chat/completions/input_tokens`; its exact multimodal `input_tokens` result drives the remaining-context budget, and endpoint mismatch fails closed without logging prompt, image, or response bodies. Source: https://github.com/ggml-org/llama.cpp/blob/b9982/tools/server/README.md#post-v1chatcompletionsinput_tokens, checked 2026-07-31.
...
- Exact Qwen3.5 artifacts send the documented `chat_template_kwargs.enable_thinking=false` option: [Qwen3.5](https://huggingface.co/Qwen/Qwen3.5-4B#instruct-or-non-thinking-mode). Gemma 4 keeps the backend model default until the exact GGUF and pinned llama.cpp combination has source-backed option evidence. `reasoning_content` is discarded and an incremental filter prevents split `<think>` traces from reaching buffered or streaming display. SSE event, HTTP chunk, and incomplete-body buffers have explicit limits. Upstream error payloads are reduced to a fixed category, and ledger details never store raw prompt or raw response text.
