...
- `rpotato backend start --model <path> [--ctx-size <tokens>]` starts the selected sidecar with an explicit local model file and optional runtime context limit, writes a pid record under app state, captures stdout/stderr to log files, waits for `/health`, and kills the child on startup timeout.
