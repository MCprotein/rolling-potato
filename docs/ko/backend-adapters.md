...
- 생성 전에 pinned `llama.cpp b9982`의 `/v1/chat/completions/input_tokens`에 동일한 text, JSON schema, chat-template option, image content를 보내 exact multimodal `input_tokens`를 얻어 remaining-context 예산에 사용합니다. endpoint 계약이 맞지 않으면 prompt·image·response body를 기록하지 않고 차단합니다. 출처: https://github.com/ggml-org/llama.cpp/blob/b9982/tools/server/README.md#post-v1chatcompletionsinput_tokens, 2026-07-31 확인.
...
- 정확히 식별된 Qwen3.5 artifact에는 문서화된 `chat_template_kwargs.enable_thinking=false` 옵션을 보냅니다: [Qwen3.5](https://huggingface.co/Qwen/Qwen3.5-4B#instruct-or-non-thinking-mode). Gemma 4는 exact GGUF와 pinned llama.cpp 조합의 option 근거가 확인될 때까지 backend model default를 유지합니다. `reasoning_content`는 폐기하고 incremental filter가 split된 `<think>` trace를 buffered/streaming display에 노출하지 않도록 막습니다. SSE event, HTTP chunk, 미완성 body buffer에는 명시적 제한이 있습니다. Upstream error payload는 고정 category로 축약하며 ledger에는 raw prompt 또는 raw response text를 기록하지 않습니다.
