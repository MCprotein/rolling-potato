...
- Qwen3.5에는 Qwen model card의 non-thinking mode guidance에 따라 `chat_template_kwargs.enable_thinking=false`를 보냅니다. `reasoning_content`는 폐기하고 incremental filter가 split된 `<think>` trace를 buffered/streaming display에 노출하지 않도록 막습니다. SSE event, HTTP chunk, 미완성 body buffer에는 명시적 제한이 있습니다. Upstream error payload는 고정 category로 축약하며 ledger에는 raw prompt 또는 raw response text를 기록하지 않습니다.
