...
v0.38.0은 local import/inspection surface와 Codex/Claude Code의
instruction-only 실행 adapter를 구현합니다.
...
- `rpotato plugin remove <id> --keep-data`
- `rpotato plugin remove <id> --purge-data`
- Enable된 instruction-only Codex/Claude Code capability를 찾는 `rpotato skill list`
...
`enable`은 일반 실행 권한 부여가 아니라 registry 상태 변경입니다. Codex
adapter는 다음 조건을 모두 만족하는 64 KiB 이하 regular file인 canonical
`skills/<name>/SKILL.md`만 admit합니다.
...
v0.38 Claude Code adapter는 canonical `skills/<name>/SKILL.md`와 flat
`commands/<name>.md` regular file만 admit합니다. Path name이 invocation
name이며 instruction body는 비어 있으면 안 됩니다. Description이 없으면 첫
Markdown paragraph를 사용합니다. 같은 이름의 skill과 command가 있으면 skill을
우선합니다. Source manifest가 custom `commands` path를 선언하면 아직 그
replacement semantic을 mapping하지 않았으므로 기본 `commands/`도 admit하지
않습니다.
...
Claude Code dynamic shell interpolation은 `shell-command`로 분류해
차단합니다. `allowed-tools`, `context`, `agent`, `hooks`, `model`, `paths`,
argument/environment substitution 같은 source-runtime frontmatter는 unsupported로
보고하며 native read-only contract를 넓힐 수 없습니다. Root `SKILL.md`, custom
component path, agent, hook, MCP, LSP, monitor, `bin/`, settings, theme,
output style, channel, dependency, user configuration도 mapping하지 않은 경우
명시적으로 보고합니다.

Shell, skill script, `bin/`, MCP, app integration, plugin hook, LSP,
monitor/background process, runtime setting, remote connector, sensitive config,
file-write capability는 permission report에 남고 계속 차단됩니다. v0.38에는 이
risky capability를 위한 approval grant 또는 실행 surface가 없으며 `plugin
enable`은 그러한 승인이 아닙니다. 이후 구현이 이를 실행하려면 capability
scope에 binding된 명시적 approval lease와 재검증을 먼저 추가해야 합니다.
...
| Claude Code skill | `rpotato` skill | Canonical default-path instruction-only skill을 native read-only runtime으로 실행 |
| Claude Code command | `rpotato` skill | Dynamic shell bypass가 없는 flat default-path prompt-backed command만 실행 |
| Claude Code agent | `rpotato` subagent role | tool/path/model field를 안전하게 mapping할 때까지 명시적 unsupported |
| Claude Code hook | `rpotato` hook | event와 hook type을 runtime hook contract로 mapping할 때까지 unsupported 및 차단 |
| Claude Code MCP server | MCP adapter entry | 보고하고 차단하며 실행 surface는 없음 |
...
7. Claude Code skill과 command import. **v0.38.0에서 canonical default path에 대해 구현.**
