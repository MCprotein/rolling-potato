...
- append-only ledger와 immutable transcript artifact: audit 소유권/순서 및 durable user/visible-model/tool/evidence turn
- SQLite projection: CLI/TUI query를 위한 재생성 가능한 session-history와 `transcript_records` view
- current-state: 현재 선택된 `session_id`와 resume metadata

`rpotato session list`, `rpotato session history`, id 없는 `rpotato resume`은 canonical artifact에서 재생성한 SQLite view를 표시합니다. `rpotato session resume <session-id>`, `rpotato resume <session-id>`, `rpotato continue <session-id>`는 current-state 변경 전에 ledger 소유권, 참조된 transcript artifact 전체, 현재 source hash, 같은 session의 workflow/proposal/evidence binding을 검증하고 안전한 workflow checkpoint만 계속합니다. 인자 없는 `continue`는 현재 session을 사용합니다. Resume transcript budget은 선택 모델 manifest에서 계산합니다. 4K window는 최대 8 turn·estimated token 512, 131K window는 최대 64 turn·estimated token 16,384까지 사용합니다. 현재 요청과 resume source context는 의도적으로 더 작은 source pointer 최대 4개·3,200자의 retrieval budget을 계속 공유합니다. Interactive TUI는 완료된 user/assistant pair를 별도의 append-only canonical conversation stream에 저장하고 `/clear`를 reset boundary로 기록합니다. Hidden reasoning/raw backend response, raw source body, patch fragment, verification command 원문은 transcript record가 아니며, 불확실한 backend request나 verification command를 자동 재실행하지 않습니다.
...
v0.30.0 local adoption gate는 측정된 Gemma artifact를 promote하고, pinned Qwen 후보가 이 host의 exact-response equality를 실패한 뒤 Gemma를 지속 기본값으로 선택했습니다. 이는 [model-eval.md](model-eval.md)에 기록된 local product 결과이며 public benchmark parity나 모든 hardware fit claim이 아닙니다.
