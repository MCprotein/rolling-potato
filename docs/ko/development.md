...
`run`은 이제 deterministic routing, intent ledger 기록, source pointer가 포함된 bounded repository context pack 생성, 실행 중인 backend sidecar 호출, agent-loop 골격, SQLite token/latency metric 기록까지 수행합니다. Patch 적용, command 실행, 승인된 action 처리는 후속 phase로 남아 있습니다.
