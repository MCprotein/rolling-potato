...
Checkpoint, ledger chain, CSPRNG token, atomic rollback, stop gate test는 플랫폼 독립적인 unit/state test이며 Windows에서도 실행됩니다. `tests/patch_loop.rs`의 실제 subprocess fixture는 실행 가능한 Python fake sidecar와 Unix process permission을 사용하므로 현재 Unix 전용입니다. 따라서 v0.29.0은 Windows subprocess end-to-end coverage를 주장하지 않습니다. Windows는 portable state/runtime boundary를 실행하고 Unix는 추가로 multi-process CLI/backend flow를 검증합니다.
...
`patch preview --path <path> --find <text> --replace <text>`는 approve/apply/verification을 할 수 없는 diff-only standalone surface입니다. Mutation 권위는 `run`이 만든 workflow/action/proposal ID, before/after hash, 정확한 verification command binding에만 있습니다. `patch approve`는 모든 binding과 fresh source hash를 검증하고 side effect 전에 approval을 저장한 뒤 no-clobber guard transaction과 rollback record로 적용합니다. Policy가 허용한 pre-bound argv verification만 실행하고 hash-only evidence를 기록하며 applied source와 evidence가 fresh할 때만 stop gate를 통과합니다. `rpotato cancel`은 verification을 재실행하지 않고 inconclusive applied phase를 명시적으로 정리합니다. Verification 실패는 original byte를 복원하고 성공/실패 보고는 deterministic 한국어 text로 생성합니다.
