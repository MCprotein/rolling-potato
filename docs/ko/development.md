...
`team status`는 read-only team-runtime admission preview입니다. 최신 SQLite resource sample을 읽어 requested/admitted lane을 표시하고 향후 dispatch가 parallel, sequential fallback, blocked 중 어디에 해당하는지 보여줍니다. `team admit --lanes <count>`는 첫 enforced gate입니다. Admission decision을 ledger/SQLite projection에 기록하고 critical pressure에서는 blocked error를 반환하지만, 아직 worker를 시작하거나 team stage를 전진시키지는 않습니다.
