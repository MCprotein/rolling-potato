...
현재 저장소는 `v0.48.0` release를 가진 active pre-1.0 Rust runtime입니다.
읽기 쉬운 기능 지도는
...
`run`은 bounded active-conversation history를 재구성하고 현재 요청과 과거 context 전체에 source pointer 최대 4개·3,200자의 단일 공유 budget을 적용한 뒤 canonical workflow를 만들고 backend를 호출합니다. 과거 source pointer는 context hint이므로 stale이면 해당 pointer만 제외하고 최근 대화는 유지하며, 현재 요청의 repository source는 별도로 최신 원문을 다시 읽습니다. History allowance는 선택 모델 manifest에서 계산합니다. 4K model은 최대 8 turn·estimated token 512, 131K model은 최대 64 turn·estimated token 16,384까지 사용합니다. 명시적인 `resume`/`continue`는 stale source hash와 안전하지 않은 workflow checkpoint를 계속 fail-closed합니다. Runtime은 user, visible/normalized model, tool, evidence transcript artifact를 canonical하게 저장하되 hidden reasoning, raw backend response, raw source body, patch fragment, verification command 원문은 제외합니다. `state resume`은 session 선택 전에 transcript/workflow/ledger checkpoint를 검증하고 pending approval에서 backend를 다시 호출하거나 불확실한 verification command를 자동 재실행하지 않습니다.
