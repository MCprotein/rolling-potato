...
`run`은 이제 deterministic routing, intent ledger 기록, source pointer가 포함된 bounded repository context pack 생성, runtime-owned action candidate와 next gate 준비, 실행 중인 backend sidecar 호출, model의 structured action line 또는 인식 가능한 action text를 실행 불가 model-action record로 파싱, SQLite token/latency metric 기록까지 수행합니다. Patch 적용, command 실행, 승인된 action 처리는 후속 phase로 남아 있습니다.

`patch preview --path <path> --find <text> --replace <text>`는 project-local text replacement 하나에 대한 unified diff를 렌더링하고 `.rpotato/patch-proposals/` 아래에 proposal을 기록한 뒤 approval token을 출력합니다. `patch approve <proposal-id> --token <token> --dry-run`은 token을 검증하고 patch를 적용하지 않은 채 approval gate event를 기록합니다. 승인된 patch apply는 후속 phase입니다.
