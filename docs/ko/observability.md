...
| v0.20.0 | small-model ontology benchmark | 같은 runtime monitoring schema로 2B-4B target model의 prompt-facing ontology view를 비교 |
...
| v0.22.0+ | 남은 dispatcher governor policy | dispatch-time ownership enforcement와 failed-worker continuation 추가 |
...
- 차단된 team admission policy/ownership decision은 `.rpotato/approval-requests/` 아래에 redacted project-local approval request record를 쓰며, `rpotato tui approvals`는 이 record를 patch proposal approval과 함께 표시한다.
