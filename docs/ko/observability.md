...
- TUI prompt 조립은 최근 typed tool activity를 model-window 상대 별도 budget으로 복원한다. 기록된 `succeeded` activity만 성공으로 제시하며 failed, blocked, cancelled status는 그대로 유지한다. 복원된 activity는 tool 재실행을 승인하거나 실행됐다고 간주하는 근거가 아니다.
