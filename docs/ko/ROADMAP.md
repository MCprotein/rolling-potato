...
| v0.37.1 | Planned | 아키텍처 기반: 영문/한국어 코드 아키텍처 정본, 전체 migration ledger, private compile-connected module skeleton, architecture contract test, 운영 로직 이동 없는 exact-head candidate CI |
| v0.37.2 | Planned | Foundation과 platform seam: filesystem, terminal, configuration, checksum, strict serialization, lease, cache, Windows atomic replacement 소유권 |
| v0.37.3 | Planned | Inference 경계: backend, model, benchmark, resource domain rule을 llama.cpp, process, filesystem adapter에서 분리 |
| v0.37.4 | Planned | Canonical workflow storage compatibility: 바이트 동일 DTO/codec과 분리된 ledger/transcript append 소유권 |
| v0.37.5 | Planned | 변경되지 않은 storage compatibility 경계를 감싼 validated workflow, session, snapshot, transcript view |
| v0.37.6 | Planned | Workflow transition, crash recovery, cross-store commit/recovery 순서를 소유하는 단일 transaction coordinator |
