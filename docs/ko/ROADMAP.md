...
| v0.40.0 | Released | Package manager 배포: exact checksum 기반 Homebrew/Scoop manifest, strict format contract, clean-install/upgrade/uninstall lifecycle을 채택한 native lane에서 검증했으며 외부 channel 게시는 별도 evidence로 추적 |
...
Winget은 채택한 roadmap 또는 publication channel이 아닙니다. v0.40.0의
generator, fixture, validation evidence는 과거 artifact로 유지할 수 있지만 이후
버전의 완료 gate에는 upstream winget 제출이나 게시가 포함되지 않습니다.
