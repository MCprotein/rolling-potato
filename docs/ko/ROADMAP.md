...
| v0.37.13 | Released | 동작을 보존한 전체 architecture ownership migration: private `app`/`composition`/`surfaces`/`runtime_core`/`adapters`/`foundation` root, 얇은 binary entrypoint, 최상위 compatibility facade 0개, 완성된 migration ledger |
