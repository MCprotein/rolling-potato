...
### 안내형 로컬 대화와 컨텍스트 연속성 (`v0.43.0`)

| Version | 상태 | 범위 |
| --- | --- | --- |
| v0.43.0 | In development | 인자 없는 안내형 TUI에 최초 model/backend 설정을 통합하고 기본 command surface와 구조화된 상태 출력을 간소화하며, 75% 자동 trigger, 수동 `/compact`, immutable incremental checkpoint, deterministic fallback을 갖춘 소형 모델용 context compaction 제공 |
