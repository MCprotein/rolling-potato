...
- `Consolidated`: 별도 공개 릴리즈 대신 이후 exact-tree release에 포함된 내부
  milestone입니다.
- `In development`: 구체적인 version과 범위는 확정했지만 release artifact는
  아직 게시하지 않은 구현 단계입니다.
...
| Version | 상태 | 범위 |
| --- | --- | --- |
| v0.42.0 | Released | GitHub Release archive self-install과 전체 관리형 uninstall: 사용자 전용 binary 배치·삭제, 멱등 zsh/bash/fish 또는 Windows 사용자 PATH 등록·해제, `init` 환경 보정, dry-run·명시적 확인·active runtime 삭제 차단 |

### 안내형 로컬 대화와 컨텍스트 연속성 (`v0.43.0`-`v0.43.1`)

| Version | 상태 | 범위 |
| --- | --- | --- |
| v0.43.0 | Superseded | 인자 없는 안내형 TUI와 소형 모델용 context compaction을 추가했지만 release-binary smoke와 Windows ConPTY fixture 실패로 complete 5-platform asset set을 게시하지 못함 |
| v0.43.1 | Released | 안내형 header에 release smoke를 맞추고 ConPTY probe parsing과 final candidate CI gate를 보강해 v0.43 동작의 complete 5-platform asset set을 복구 |

### 관리형 업데이트와 project-safe 시작 (`v0.44.0`-`v0.45.1`)

| Version | 상태 | 범위 |
| --- | --- | --- |
| v0.44.0 | Released | 새 stable release를 알리는 non-blocking cached startup check와 관리형 사용자 설치본을 platform-exact archive 및 SHA-256 검증으로 교체하는 one-command self-update |
| v0.45.0 | Released | 공유 설치에서 project별 TUI current-state 격리, 제한된 legacy pointer 이전, canonical ledger ancestor 검증, 정확한 startup error 표시 |
| v0.45.1 | Released | 정상 legacy-v1-to-chained-v2 runtime ledger의 TUI read-only 호환성을 복구하면서 digest, chain, head count, 변조, bounded-read 검증을 유지 |

### 대화 interface 개선과 신뢰성 복구 (`v0.46.0`-`v0.46.2`)

| Version | 상태 | 범위 |
| --- | --- | --- |
| v0.46.0 | Released | Compact welcome, 지속 bordered composer, 의미별 status segment, terminal-cell 줄바꿈, 긴 응답의 무손실 page 이동, cross-platform exact terminal outcome 경계로 기본 대화 TUI를 개선 |
| v0.46.1 | Superseded | 실제 기본 대화를 복구했지만 live update check를 사용한 release PTY gate가 platform asset build 전에 timeout됨 |
| v0.46.2 | Released | Network와 분리한 deterministic PTY gate와 연쇄 실패를 막는 terminal-test 보고로 v0.46.1 대화 수정 사항을 게시 |

### 범용 답변과 웹 근거 (`v0.47.x`)

| Version | 상태 | 범위 |
| --- | --- | --- |
| v0.47.0 | Released | 일반 로컬 모델 답변·계산, 안정된 rpotato 정체성, 혼합 언어 재작성, 최신성 자동 routing, HTTPS 출처 URL을 표시하는 제한된 읽기 전용 `/search` |
| v0.47.1 | Released | 일반 설명 routing 안정화, 검색·업데이트 명령 노출, 소형 모델용 근거 제한, 혼합 언어 누출 방지, runtime 소유 출처 표시 |

### 대화형 로컬 에이전트 도구 (`v0.48.0`)
...
### 제한된 웹 연구, browser interaction과 TUI 신뢰성 (`v0.50.x`)
...
### 모델 기반 생성과 터미널 완성도 (`v0.54.0`-`v0.55.2`)
...
| v0.55.1 | Released | 검증된 각 웹 단계 뒤 model→tool→observation 재판단을 반복하되, runtime 소유 출처 결합, 관찰에서 발견한 URL·현재 page만 허용하는 admission, 동일 호출 거절, 도구 3회·후속 model turn 3회 상한을 적용하는 보정 patch |
| v0.55.2 | Released | 성공한 읽기 전용 도구 observation을 요구하는 근거 기반 project-local 답변, 분리된 짧은 결정·visible reply budget, 요청 전체 deadline 보존, multicore backend pressure 정규화와 production-terminal `Cargo.toml` 읽기 회귀 검증 |
