...
gate는 [release-train.md](release-train.md)에 정의합니다. 현재 버전의 release
차단 기반 보정은 [v0.29-correction-plan.md](v0.29-correction-plan.md)에서
추적합니다.
...
| v0.29.0 | In Progress | 지속 가능한 single-agent runtime 보정: restart-safe patch loop를 보존하면서 범용 workflow/action 경계, 비패치 `run`, ontology-backed context, 분리된 patch/verification 승인, canonical ledger 권위, deterministic 한국어 보고를 복구 |
| v0.30.0 | Planned | 검증된 모델 도입: 출처가 기록된 Qwen/Gemma 후보 평가를 완료하고 local backend/RAM/mmproj/benchmark evidence를 기록하며 install gate를 통과한 후보만 승격해 model weight를 release에 포함하지 않는 managed model install/default selection 제공 |
