...
| v0.40.0 | In progress | Package manager 배포: exact GitHub Release checksum에서 Homebrew/Scoop/winget manifest를 `Generated` 상태로 만들고 static contract를 통과했으며 native 6개 lane의 `Validated` 상태는 대기 중이고 외부 channel은 모두 `Unpublished` |
