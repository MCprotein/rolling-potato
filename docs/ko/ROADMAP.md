...
| v0.29.0 | In Progress | 지속 가능한 single-agent patch loop 구현: `run`이 canonical workflow/action state를 저장하고 source를 다시 읽어 proposal을 만든 뒤 binding된 승인, guarded apply, verification evidence, stop gate, deterministic 한국어 보고까지 재개 |
