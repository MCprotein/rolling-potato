...
| v0.37.13 | Implemented | Surface와 composition 완료: CLI/TUI 소유권, startup/dispatch wiring, uninstall orchestration, `app` 아래 application adapter, 얇은 `main`/private `lib` entry facade, 최상위 compatibility facade 0개, migration ledger 전 범위 완료; release 대기 |
