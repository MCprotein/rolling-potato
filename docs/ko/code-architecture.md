...
`main.rs`는 private `lib.rs` facade에 위임하고, `app`은 concrete application
orchestration과 port-to-adapter integration을 소유하며, `composition`은 startup,
...
`src` 최상위에는 `main.rs`와 `lib.rs`만 있습니다. Production compatibility
facade는 남아 있지 않습니다. Root Cargo integration harness는
...
lib facade -> app + composition + private root
