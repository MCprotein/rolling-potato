...
```text
src/
  main.rs                     private root와 얇은 binary entrypoint
  app/                        concrete application adapter와 integration
  ...
  runtime_core/
    ...
    knowledge/                 context, 대화 recall, prompt budget,
...
노출합니다. v0.37.1-v0.37.12 patch는 각 ledger slice를 targeted test와 legacy
path 제거가 함께 닫힐 때만 이동했습니다. v0.37.13은 최상위 정리를 완료했습니다.
Binary가 소유하는 `main.rs`는 architecture root를 private으로 등록하고
`composition::startup`에 직접 위임합니다. `app`은 concrete application
orchestration과 port-to-adapter integration을 소유하고, `composition`은 startup,
CLI dispatch, inference, TUI action/read, uninstall wiring을 소유합니다. Domain
rule은 `runtime_core`, 사용자 interface는 `surfaces`, side effect는 `adapters`,
capability와 독립된 primitive는 `foundation`에 유지합니다.

`src` 최상위에는 `main.rs`만 있고 모든 production owner는 이름이 있는
하위 directory에 있습니다. Compatibility facade는 남아 있지 않습니다. Root Cargo integration harness는
`tests/collaboration` 아래 contract 같은 grouped test body에 위임할 수 있지만,
production 소유권을 crate root로 되돌리지 않습니다.
