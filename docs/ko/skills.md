...
Phase 3의 현재 구현은 skill 실행 전 정규화 단계이며, `rpotato run`은 이 routing state를 사용해 첫 model-response-only agent-loop 골격까지 진입합니다.
...
- `rpotato run "<request>"`는 deterministic intent rule로 user request를 skill/mode로 매핑하고, intent/backend chat ledger event를 남기며, 실행 중인 backend sidecar를 호출하고 token/latency metric을 기록한다.
...
- Repository context packing, tool call, 승인된 patch 적용, verification command 실행은 후속 phase에서 처리한다.
