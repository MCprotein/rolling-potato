...
- `rpotato run "<request>"`는 deterministic intent rule로 user request를 skill/mode로 매핑하고, source pointer가 포함된 bounded repository context pack을 만들며, intent/context/backend chat ledger event를 남기고, 실행 중인 backend sidecar를 호출해 token/latency metric을 기록한다.
