...
| v0.21.0 | Benchmark-driven optimization policy | `rpotato monitor optimize`가 측정된 local metric과 benchmark evidence로 context budget, lane count, fallback, model route를 추천 |
...
- v0.20.0 executable report는 `model_run_id`, prompt artifact checksum, prompt length,
  ...
  peak RSS를 추가한다.
- v0.21.0 optimization report는 `measured-locally` benchmark row와 local
  performance metric만 소비한다. 출력은 runtime hint이며 model 승격이나 public
  benchmark parity claim이 아니다.
