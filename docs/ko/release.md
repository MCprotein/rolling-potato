...
`cargo test --locked -- --test-threads=1 --skip backend::tests::start_timeout_removes_record_and_keeps_logs`와
`scripts/release/verify-release-target-matrix.sh`를 통과시킵니다. Local release verification은
release branch merge 전에 sidecar timeout fixture를 포함한 전체 `cargo test --locked` suite를
계속 실행합니다.
