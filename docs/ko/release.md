...
`rpotato uninstall --clean --dry-run`은 설치 binary, 소유 PATH 등록, 전역
application data, 현재 project state를 변경 없이 보여 줍니다.
`rpotato uninstall --clean --yes`는 같은 runtime guard 아래 이 관리형 target을
삭제합니다. 압축을 푼 실행 원본과 source repository는 사용자 소유로
보존하며 Windows self-delete는 process 종료 뒤 완료합니다.
