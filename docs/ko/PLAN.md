...
- GitHub repository
- binary용 GitHub Releases. v0.24.1 workflow는 macOS Apple Silicon, macOS Intel,
...
- source-recorded `Qwen3.5-4B` Q4_K_M artifact가 local `llama.cpp b9878` smoke, RAM-fit, mmproj 필요 여부, benchmark 검사를 통과하는가?
...
1. source-recorded Qwen/Gemma GGUF artifact 후보의 local smoke와 RAM-fit/mmproj 필요 여부를 확인한다.
2. 이어받기 가능한 model download와 registry 등록 전 byte-level SHA-256 검증을 구현한다.
3. disk의 초기 model manifest format을 정의한다.
4. runtime core module을 CLI surface에서 분리한다.
5. plugin 실행 전에 normalized plugin manifest와 inspect/validate output을 정의한다.
6. Claude Code plugin local import보다 Codex plugin local import를 먼저 구현한다.
7. 위험한 foreign plugin capability는 capability별 명시 승인 전까지 차단한다.
8. plugin marketplace, remote registry, remote catalog source를 거부한다.
9. agent behavior보다 `rpotato doctor`를 먼저 구현한다.
10. 한국어/code/tool reliability를 위한 첫 fixture benchmark를 만든다.
