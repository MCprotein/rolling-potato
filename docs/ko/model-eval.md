...
- `model default <id>`는 재검증된 registry 모델만 지속 선택합니다. 이 문서의 historical Gemma record는 local promotion evidence를 사용합니다. Interactive setup은 source/license/backend source와 artifact bytes를 재검증한 뒤 제한된 `source-backed-manifest` evidence 상태를 대신 지속할 수 있으며, 이 상태는 RAM, 품질, benchmark claim을 만들지 않습니다. `backend start`에서 `--model`을 생략하면 해당 evidence 상태를 다시 검증한 뒤에만 선택을 사용합니다.
