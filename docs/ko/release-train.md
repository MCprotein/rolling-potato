...
발행하지 않은 `v0.37.1`부터 `v0.37.12`까지의 row는 별도 출시 artifact가
아니라 구현 milestone입니다. Maintainer가 승인한 전체 train squash는 이들의
정확한 최종 tree를 `v0.37.13` candidate로 merge했습니다. 따라서 publication은
하나의 `v0.37.13` release branch와 tag를 사용합니다. 동일한 완성 구현으로 여러
tag를 만드는 것은 artifact history를 잘못 나타냅니다. 이 consolidation은 이미
완료된 v0.37.x architecture train에만 적용하며 이후 roadmap version의
one-version release cycle을 약화하지 않습니다.
...
| `v0.37.2` | Private foundation/filesystem/terminal owner, 완료 slice compatibility facade 0건, 통합 platform contract, targeted owner test, exact-head candidate CI |
...
- Winget은 채택한 roadmap 또는 publication channel이 아닙니다. 과거 v0.40.0
  generator와 validation artifact는 upstream 제출을 승인하거나 요구하지 않습니다.
