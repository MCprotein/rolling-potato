...
모두 구현하고 출시하는 Ultragoal 실행 계약입니다.
...
Aggregate Goal은 `v0.41.0` 출시, 이전 모든 버전 gate 통과, 최종 Ultragoal
quality gate 통과, 미해결 blocker story 0건이 모두 충족된 경우에만
완료됩니다.
...
11. Merge된 local/remote release branch를 제거하고 구체적인 evidence로
    Ultragoal story를 checkpoint한 뒤 다음 버전을 시작합니다.
...
Ultragoal steering은 evidence가 있을 때 work split, blocker-resolution story
추가, pending work 재정렬, pending wording 수정만 허용합니다. Gate 완화,
완료 조작, 조용한 version skip, aggregate product objective 변경은 금지합니다.

## 최종 Quality Gate

Aggregate Goal을 완료하기 전에 다음을 실행합니다.

1. `v0.41.0`과 전체 release history를 검증합니다.
2. Ultragoal이 요구하는 changed-file cleanup pass를 실행합니다.
3. 전체 verification suite를 다시 실행합니다.
4. 이 문서, `PLAN.md`, `ROADMAP.md`, runtime architecture 문서에서 도출한
   invariant를 증명합니다.
5. 서로 다른 독립 `code-reviewer` APPROVE와 `architect` CLEAR evidence를
   확보합니다.
6. Structured final quality gate를 기록한 뒤에만 Codex Goal을 완료합니다.
