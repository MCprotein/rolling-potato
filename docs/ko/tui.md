...
일반 interactive 명령은 `/model`, `/compact`, `/search <질문>`, `/attach <경로>`,
`/update`, `/status`, `/sessions`, `/doctor`, `/more`, `/back`, `/clear`, `/help`,
`/quit`입니다. `/more`와 `/back`은 화면 밖의
...
다음 요청에 포함합니다. PNG/JPEG 이미지는 최대 4개, 합계 20 MiB까지 허용하며
dispatch 시 signature와 SHA-256을 다시 검증합니다. 선택한 model의 별도 고정
...
`llama-server`를 시작할 때 `--mmproj`로 전달합니다.

명확한 저장소·action signal이 없는 일반 질문은 가벼운 범용 답변 경로를 사용합니다.
명시적 검색 요청, 최신성이 필요한 질문, `검색해서`/`찾아봐` 같은 자연스러운 한국어
표현, `/search`는 공개 검색 HTML을 직접 요청·파싱해 제한된 snippet을 가져오고
출처 URL을 덧붙입니다. API key, 별도 MCP process, provider SDK, background 검색
service를 사용하지 않습니다. HTTPS redirect는 허용하지 않습니다. 검색 결과는
신뢰하지 않는 prompt context일 뿐 command 실행, 파일 수정, runtime 권한 확대를
할 수 없습니다. 해당 요청에서 offline/no-browse를 지시하면 자동 retrieval을
사용하지 않습니다. `/doctor`는 별도 credential 없이 검색을
사용 가능, key 미설정, alias 충돌 중 하나로 표시합니다.
