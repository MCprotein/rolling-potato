...
일반 interactive 명령은 `/model`, `/compact`, `/search <질문>`, `/update`, `/status`, `/sessions`,
`/doctor`, `/more`, `/back`, `/clear`, `/help`, `/quit`입니다. `/more`와 `/back`은 화면 밖의
긴 응답 line을 버리지 않고 page 단위로 이동합니다. `/`를 입력하면 Enter 전에 `/help`와
같은 command registry를 사용하는 live command palette가 열립니다. `/update`는 확인을 받은 뒤 현재
...
명시적 검색 요청, 최신성이 필요한 질문, `/search`는 Exa hosted MCP service에서
