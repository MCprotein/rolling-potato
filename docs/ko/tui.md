...
다음 요청에 포함합니다. 이미지는 20 MiB까지 signature와 SHA-256을 검증하지만 현재
검증된 model set은 text-only이므로 vision artifact와 `mmproj` 지원 전에는 model
dispatch 전에 capability 이유를 표시하고 차단합니다.
