...
`rpotato session list`, `rpotato session history`, and bare `rpotato resume` render SQLite views rebuilt from canonical artifacts. `rpotato session resume <session-id>`, `rpotato resume <session-id>`, and `rpotato continue <session-id>` validate ledger ownership, every referenced transcript artifact, current source hashes, and the matching workflow/proposal/evidence bindings before changing current state. They then continue only a matching safe workflow checkpoint. Bare `continue` uses the current session. Resume derives its transcript budget from the selected model manifest: a 4K window keeps up to 8 turns within 512 estimated tokens, while a 131K window can keep up to 64 turns within 16,384 estimated tokens. Current-request and resumed source context still share the deliberately smaller retrieval limit of 4 pointers and 3,200 characters. The interactive TUI separately stores completed user/assistant pairs in an append-only canonical conversation stream and records `/clear` as a reset boundary. Hidden reasoning/raw backend responses, raw source bodies, patch fragments, and verification-command text are not transcript records. Uncertain backend requests and verification commands are never automatically repeated.
