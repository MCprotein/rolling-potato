...
`team status` is the read-only team-runtime admission preview. It reads the latest SQLite resource sample, reports requested/admitted lanes, and shows whether a future dispatch would be parallel, sequential fallback, or blocked. `team admit --lanes <count>` is the first enforced gate: it records the admission decision in the ledger/SQLite projection and returns a blocked error on critical pressure, but still does not start workers or advance team stages.
