...
`team status` is the first team-runtime admission surface. It reads the latest SQLite resource sample, reports requested/admitted lanes, chooses parallel admission, sequential fallback, or blocked dispatch, and stays read-only until the real team dispatcher exists.
