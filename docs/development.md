...
`run` now performs deterministic routing, appends intent ledger state, calls the running backend sidecar for a model-response-only agent-loop skeleton, and records token/latency metrics in SQLite. Repository file reads, context packing, patch application, command execution, and approved action handling remain future phases.
