...
`run` now performs deterministic routing, appends intent ledger state, builds a bounded repository context pack with source pointers, prepares a runtime-owned action candidate plus next gate, calls the running backend sidecar for an agent-loop skeleton, and records token/latency metrics in SQLite. Patch application, command execution, and approved action handling remain future phases.
