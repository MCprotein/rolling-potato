...
`run` reconstructs bounded active-conversation history and applies one shared 4-pointer/3,200-character source budget across the current request and historical context before creating a canonical workflow or calling the backend. Historical source pointers are context hints: stale pointers are omitted while recent dialogue remains available, and the current request rereads current repository sources independently. The history allowance comes from the selected model manifest: 4K models use up to 8 turns/512 estimated tokens and 131K models can use up to 64 turns/16,384 estimated tokens. Explicit `resume`/`continue` remains fail-closed for stale source hashes and unsafe workflow checkpoints. The runtime persists canonical user, visible/normalized model, tool, and evidence transcript artifacts while excluding hidden reasoning, the raw backend response, raw source bodies, patch fragments, and verification-command text. `state resume` validates transcript/workflow/ledger checkpoints before session selection and does not call the backend again while approval is pending or automatically rerun an uncertain verification command.
