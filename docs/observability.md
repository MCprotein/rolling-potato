...
- Each immutable artifact binds its source boundary record, artifact hash, previous checkpoint hash, typed checkpoint, and complete recent exchanges selected by the model-window policy. The retained tail scales from 2 to 8 exchanges within 512 to 16,384 estimated tokens; the serialized safety ceiling is 64 records. The original transcript is never rewritten or deleted.
...
- Runtime core derives history limits from the selected model manifest. A 4K window permits up to 8 turns/512 estimated tokens, while a 131K window permits up to 64 turns/16,384 estimated tokens. Ordinary requests omit stale historical source hints and continue with current repository context; explicit session/workflow resume keeps strict source validation. It applies one shared 4-pointer/3,200-character source budget across current-request and historical context before creating or continuing a workflow.
- TUI prompt assembly restores recent typed tool activity under its own model-window-relative budget. Only recorded `succeeded` activity is presented as successful; failed, blocked, and cancelled status remains explicit, and restored activity never authorizes or implies tool replay.
