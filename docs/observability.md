...
- Runtime core derives resume limits from the selected model manifest. A 4K window permits up to 8 turns/512 estimated tokens, while a 131K window permits up to 64 turns/16,384 estimated tokens. It applies one shared 4-pointer/3,200-character source budget across current-request and resumed context before creating or continuing a workflow.
