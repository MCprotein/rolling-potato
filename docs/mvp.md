...
범위는 의도적으로 좁힙니다. 좋은 대화형 코딩 에이전트 전체가 아니라, 로컬 모델 설치부터 작은 patch 검증까지의 한 흐름을 완성합니다.
...
2. CLI가 환경을 점검한다.
3. CLI가 관리형 `llama.cpp` sidecar를 설치 또는 확인한다.
4. CLI가 출처와 checksum이 검증된 모델 후보를 제안한다.
5. 사용자가 다운로드를 승인한다.
6. CLI가 모델을 다운로드하고 hash를 검증한다.
7. 사용자가 프로젝트에서 `rpotato run "테스트 실패 고쳐줘"`를 실행한다.
8. runtime이 관련 파일을 읽고 작은 수정안을 만든다.
9. CLI가 diff를 보여주고 적용 승인을 받는다.
10. 사용자가 승인하면 patch를 적용한다.
11. CLI가 검증 명령 실행 승인을 받는다.
12. 검증 결과를 해석하고 한국어로 최종 보고한다.
...
- `rpotato model install <id>`는 다운로드 전 크기와 license 정보를 보여준다.
- 다운로드는 중단 후 재개할 수 있다.
...
- runtime은 `llama.cpp` sidecar를 시작하거나 기존 프로세스를 재사용한다.
...
- runtime은 현재 프로젝트 내부 파일을 읽을 수 있다.
