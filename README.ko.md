...
`run`은 user request를 skill/mode/context/evidence 요구사항으로 정규화하고, source pointer가 포함된 bounded repository context pack을 만든 뒤 실행 중인 backend sidecar를 호출하는 agent-loop 골격까지 수행합니다. Intent, context, backend chat ledger event와 token/latency metric을 local SQLite observability projection에 기록합니다. 아직 patch 적용, command 실행, model output의 승인된 action 처리까지는 수행하지 않습니다.
