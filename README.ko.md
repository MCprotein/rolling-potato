...
공식 binary download는 GitHub Releases로 배포합니다. v0.28.0부터 release workflow는 macOS Apple Silicon(`aarch64-apple-darwin`), macOS Intel(`x86_64-apple-darwin`), Linux x86_64(`x86_64-unknown-linux-gnu`), Linux ARM64(`aarch64-unknown-linux-gnu`), Windows x86_64(`x86_64-pc-windows-msvc`)용 `rpotato` archive를 만들고, basename-only `.sha256` checksum file과 aggregate checksums file을 생성하며, asset upload 전에 packaged binary smoke test를 실행합니다.
