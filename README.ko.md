...
v0.52.1 patch는 JSON schema repetition bound를 pinned grammar compiler에서
검증한 범위로 유지하여 관리형 `llama.cpp`의 기본 대화 경로를 복구합니다.
...
게시된 `v0.52.1`까지의 release history에는 로컬 coding-agent workflow를
