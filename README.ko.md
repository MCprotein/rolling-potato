...
`backend doctor`는 관리형 `llama.cpp` sidecar discovery, env override path, port, health URL, executable bit, install gate를 보여줍니다. `backend install-plan`은 release URL/checksum이 없으면 다운로드를 차단하고, `backend verify-archive`는 로컬 backend archive SHA-256을 검증합니다. `backend health-check`는 선택된 host/port의 `/health`를 짧은 timeout으로 진단합니다. 보안상 unknown binary를 실행하지 않으므로 version detection은 아직 수행하지 않습니다.
