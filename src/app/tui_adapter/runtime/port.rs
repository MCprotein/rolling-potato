//! Production implementation of the presentation-owned TUI runtime port.

use crate::app::tui_adapter::{
    canonical_dispatch_intent, canonical_gate_descriptor, canonical_read_page,
    canonical_selection_lease, session_memory,
};
...
use super::{backend, model_setup, request, status, web_sources, TuiRuntimeAdapter};
...
impl TuiRuntimePort for TuiRuntimeAdapter {
    ...
    fn reconcile_existing_backend(&mut self) -> Result<(), AppError> {
        backend::reconcile_existing_runtime()
    }
    ...
    fn clear_conversation_history(&mut self) -> Result<(), AppError> {
        ...
        session_memory::clear(self.conversation_memory()?)
    }
    ...
    fn submit_request_with_progress(
        ...
        let result = match execution {
            Ok(execution) => {
                let recorded = session_memory::record_exchange_with_tool_activities(
                    &mut memory,
                    request.trim(),
                    &execution.response,
            ...
            Err(error) => {
                if cancellation.is_cancelled() {
                    let recorded =
                        session_memory::record_tool_activities(&mut memory, &tool_activities);
                    self.conversation_memory = Some(memory);
                    return recorded.and(Err(cancellation.cancelled_error()));
                }
                let recorded = session_memory::record_failure_with_tool_activities(
                    &mut memory,
                    request.trim(),
                    &error.message,
                    &tool_activities,
