    ...
    connection
        .execute(
            ...
            params![
                ...
                metric.evidence_schema_version.map(i64::from),
                metric
                    .generation_status
                    .map(|status| status.storage_label()),
                metric.finish_reason,
                metric.generation_profile_fingerprint,
                metric.claim_state,
                metric.score,
                metric.score_unit,
    ...
    let rows = statement
        .query_map([], |row| {
            Ok(BenchmarkRunReport {
                ...
                evidence_schema_version: option_i64_to_u32(row.get(9)?),
                generation_status: row
                    .get::<_, Option<String>>(10)?
                    .as_deref()
                    .and_then(BenchmarkGenerationStatus::from_storage_label),
                finish_reason: row.get(11)?,
                generation_profile_fingerprint: row.get(12)?,
                claim_state: row.get(13)?,
                score: row.get(14)?,
                score_unit: row.get(15)?,
                local_pass: option_i64_to_bool(row.get(16)?),
                expected_matches: option_i64_to_u32(row.get(17)?),
                expected_total: option_i64_to_u32(row.get(18)?),
                forbidden_matches: option_i64_to_u32(row.get(19)?),
                harness_ref: row.get(20)?,
                dataset_ref: row.get(21)?,
                backend_id: row.get(22)?,
                latency_ms: row.get(23)?,
                tokens_per_second: row.get(24)?,
                prompt_tokens: option_i64_to_u32(row.get(25)?),
                completion_tokens: option_i64_to_u32(row.get(26)?),
                total_tokens: option_i64_to_u32(row.get(27)?),
                resource_pressure: row.get(28)?,
                peak_rss_bytes: option_i64_to_u64(row.get(29)?),
                reproducibility_manifest: row.get(30)?,
                redacted_report: row.get(31)?,
                recorded_at_ms: row.get(32)?,
            })
        })
        .map_err(sql_error(
