...
reject_unapproved_action_refs() {
  ...
  awk \
    ...
    -v homebrew="$homebrew_setup_pin" '
      ...
      /uses: Homebrew\/actions\/setup-homebrew@/ && index($0, homebrew) == 0 { bad = 1 }
      END { exit bad }
...
homebrew_setup_pin="Homebrew/actions/setup-homebrew@df4b09108a1de9d6f995fe68f302b3f68bd6d2ef # 2026-07-18"
...
expect_count "$package_manager_workflow" "$download_pin" 4
expect_count "$package_manager_workflow" "$homebrew_setup_pin" 1
