...
require_line "$scoop" 'git -C $seed add --renormalize bucket/rpotato.json'
require_line "$scoop" 'if ($LASTEXITCODE -ne 0) { throw "failed to stage current Scoop manifest" }'
require_line "$scoop" 'git -C $seed diff --cached --quiet -- bucket/rpotato.json'
require_line "$scoop" 'if ($LASTEXITCODE -eq 0) { throw "current Scoop manifest did not create a staged change" }'
require_line "$scoop" 'if ($LASTEXITCODE -ne 1) { throw "failed to inspect staged current Scoop manifest" }'
