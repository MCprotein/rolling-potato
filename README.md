...
Backend CPU/RSS/disk resource sampling is available through `backend start`, `backend status`, `backend chat`, `monitor status`, and the read-only `tui monitor` resource-pressure panel. The first runtime resource governor slice is active for backend chat. `team status` is a read-only admission preview that uses the latest resource sample to choose parallel admission, sequential fallback, or blocked dispatch before the real team dispatcher exists.
