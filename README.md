...
rpotato skill run imported.codex.my-plugin.review "review this repository"
rpotato team status
...
Plugin adapters do not use external marketplaces. Users import local Codex/Claude Code-style plugin directories, and `rpotato` inspects, validates, and enables normalized capabilities after reporting permission risks. An enabled Codex plugin may expose canonical instruction-only `skills/<name>/SKILL.md` entries through `skill list` and `skill run imported.codex.<plugin>.<skill>`. The runtime revalidates the imported snapshot and frontmatter at execution time and keeps the capability read-only. Skill scripts, hooks, MCP servers, app integrations, shell/background processes, remote connectors, and write capabilities remain blocked; plugin enablement is not approval to execute them.
...
`intent classify` and `intent routes` remain pre-execution surfaces. `skill run <id> "<request>"` explicitly selects a built-in skill or an enabled instruction-only Codex/Claude Code skill or command and enters the same durable agent loop as `run`, including context checks, lifecycle hooks, runtime policy, evidence collection, and stop criteria. Imported instructions are untrusted prompt content and cannot widen the runtime-owned action contract, tool policy, approval boundary, Korean guard, evidence requirements, or stop gate.
