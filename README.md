...
`intent classify` and `intent routes` remain pre-execution surfaces. `skill run <id> "<request>"` explicitly selects a built-in skill or an enabled instruction-only Codex skill and enters the same durable agent loop as `run`, including context checks, lifecycle hooks, runtime policy, evidence collection, and stop criteria. Imported instructions are untrusted prompt content and cannot widen the runtime-owned action contract, tool policy, approval boundary, Korean guard, evidence requirements, or stop gate.
...
`plugin import` accepts only local Codex/Claude Code-style plugin directories. It snapshots the source into app data, records source manifest and source snapshot SHA-256 hashes in a normalized schema v2 manifest, maps visible capabilities, and reports required and blocked permissions. `plugin validate` and `plugin enable` re-check the imported snapshot hash and mark the plugin `blocked` on drift. v0.37 executes only enabled canonical Codex `SKILL.md` capabilities whose frontmatter and instruction body validate and whose skill directory contains no permission-requiring capability. Each run records plugin admission/completion with the source SHA-256 and uses the native read-only hook/skill/evidence/stop-gate path. Import and enable never grant shell, script, MCP, app, hook, background, runtime-setting, remote-connector, sensitive-config, or file-write execution authority by themselves.
